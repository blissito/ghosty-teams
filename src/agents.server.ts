...
import { extractTodos, renderTodosBlock, stripStepsBlock, stripToolBlock, type TodoItem } from "./lib/ebdoc";
...
const ACP_ENTREGA = [
  ...
  "IMÁGENES: antes de decir que no puedes, MIRA si tienes el SDK — `ls /opt/gs-sdk/image.mjs`. Si está, SÍ generas imágenes: `image.generate(prompt)` → `image.publish(bytes)` → entrégala como ![descripción](url) en tu respuesta. Sirve para avisos, carteles, pósters, señalética, infografías e ilustraciones, y es SIEMPRE mejor que dibujarlas a mano con HTML y <div>. Si el archivo no existe, o si falla por falta de OPENAI_API_KEY, entonces sí dilo tal cual y no lo sustituyas en silencio por un HTML maquetado a mano.",
  "LO QUE NO PUEDES HACER, y no cambia según dónde te escriban: no generas notas de voz ni usas herramientas de Ghosty que no tengas cargadas. No depende del canal, del hilo ni del DM — depende de tu instalación. Así que NUNCA propongas que te lo pidan «en otro canal», «en un DM» o «donde tengas la voz habilitada»: no hay ningún sitio donde puedas, y mandar a alguien a intentarlo es peor que decir que no.",
...
const EB_DOC_STREAM_GUARDRAIL = [
  ...
  "eb-doc — JERARQUÍA: esto es una HOJA, no una página web. **El cuerpo arranca con el contenido, sin encabezado de título**: en una carta u oficio es la ciudad y la fecha; en un escrito, su primer apartado. El NOMBRE del documento vive en el título del fence — es lo que se ve en la tarjeta, en la pestaña del panel y en las propiedades del .docx —, así que repetirlo como encabezado sólo estorba en la hoja impresa. Las secciones van en `###` y de ahí no bajes (salvo el memo/informe de pantalla, abajo). **Nunca `#` ni `##`** en carta o escrito: son tamaño de portada, se comen media hoja y al imprimir parten el título en dos líneas. Las cláusulas y apartados numerados NO son encabezados: van en negrita al inicio de su párrafo (`**PRIMERA.** El arrendatario…`, `**SEGUNDO.** Tener por acreditada…`), que es como se ve un escrito de verdad. Reserva las listas para enumeraciones reales (datos de prueba, anexos), no para el cuerpo.",
  // Estilo «Notion» para lo que se LEE EN PANTALLA (memo, brief, informe, análisis,
  // propuesta, comparativa). Calcado el 2026-09-16 de cómo redacta Claude Docs: veredicto
  // arriba, tablas para comparar, pasos numerados, checklist de siguientes pasos, fuentes al
  // pie. Todo eso lo parsea BlockNote a bloques nativos (table, checkListItem, quote):
  // medido con mdToBlocks. Es OTRO género que la carta/escrito de arriba: allá manda la hoja
  // impresa; aquí manda el escaneo rápido.
  "eb-doc — MEMO / BRIEF / INFORME / ANÁLISIS / PROPUESTA / COMPARATIVA (lo que se lee en pantalla, no lo que se firma) va en estilo Notion, en este orden: (1) **una línea de contexto** en cursiva con la fecha y para quién es (`*16 de septiembre de 2026 · para Brenda*`); (2) **el veredicto en UN párrafo** antes de cualquier encabezado — qué se puede hacer hoy y qué falta, en dos oraciones; quien lea sólo eso ya se lleva la respuesta; (3) secciones en `##` cortas, cada una abre con una frase que resume y sigue con la evidencia; (4) **toda comparación va en TABLA** (`| Pieza | Hoy | En gs |`), con la primera fila como encabezado y celdas de una línea con `código` y **negrita** para lo que importa — nunca una lista de viñetas donde cabe una tabla; (5) los pasos van en lista numerada, uno por línea, con verbo al frente; (6) cierra con **Siguientes pasos como checklist** (`- [ ] gs: decidir…`), cada ítem con dueño y verbo; (7) última línea `Fuentes:` con qué se leyó y cuándo. Sin introducciones («en este documento…»), sin conclusiones que repitan, sin viñetas de una palabra. Aquí `##` SÍ va (es pantalla, no hoja): la regla de `###` es para cartas y escritos.",
  ...
  "· IMAGEN / FOTO / ILUSTRACIÓN / LOGO / AVISO / CARTEL / PÓSTER / LETRERO / SEÑALÉTICA / INFOGRAFÍA / GRÁFICO DE CAMPAÑA — todo lo que se MIRA en vez de leerse, incluido «diseña un aviso», «algo visual para pegar», «que sea ilustrativo» → un PNG real generado con `image.generate()` de /opt/gs-sdk/image.mjs y publicado con `image.publish()`, entregado como ![descripción](url). NUNCA un SVG dibujado a mano, ni un eb-artifact, ni HTML con gradientes y <div>: eso sale amateur y no se puede imprimir. Si además lo quieren imprimible, mete esa MISMA url en un pdf-doc.",
  "· Si `image.generate` falla por falta de OPENAI_API_KEY, DILO —«no tengo generación de imágenes habilitada en esta cuenta»— y ofrece el pdf-doc como plan B. No lo sustituyas en silencio por un HTML dibujado a mano.",
...
const TEAMS_PRODUCT_CONTEXT = [
  ...
  "TRABAJOS LARGOS (varios pasos, minutos u horas): 1) Arranca escribiendo tu PLAN con tu herramienta de plan (TodoWrite o update_plan, la que tengas) y mantenlo al día —marca cada tarea en curso y hecha en cuanto cambie—: la plataforma lo pinta como la lista viva del hilo y es lo primero que la persona mira para saber en qué vas. 2) Entrega cada avance en cuanto exista con `await run('chat_post', { text })` (mismo `connectors.mjs`): un mensaje corto por hito —un hallazgo, un PR abierto, un archivo listo—, sin esperar al final. Nunca uses `chat_post` para anunciar lo que vas a hacer ni para la respuesta final. 3) Si algo que ya publicaste resultó falso o quedó viejo, corrígelo EN SU LUGAR con `await run('chat_edit', { messageId, text })`: tacha lo que ya no vale con ~~así~~ y agrega «[Edit: …]»; no mandes un mensaje nuevo que contradiga al anterior. 4) La respuesta final cierra con el resultado y la cuenta de lo hecho (cuántos, qué falta); menciona a la persona con @ sólo si necesitas que haga algo (aprobar, decidir) — esa mención le llega como aviso.",
...
export async function callAgentBackendStream(
  ...
  if (agent.backend.kind === "acp") {
    ...
    const studio = agent.backend.id
      ? await (await import("./server/studio-prompt.server")).studioIdentity(agent.backend.id)
      : null;
    const base = studio?.prompt ?? null;
    const identidad = [
...
let enginesCache: { at: number; byId: Map<string, string>; modelById: Map<string, string> } | null = null;
  ...
  try {
    ...
    if (base) {
      ...
      for (const p of await listNativeFleetAgents(base, "")) {
        ...
        if (p.model) modelById.set(p.id, p.model);
      }
    }
  } catch {
    // Sin lista no hay hermano mayor: se cae a ofrecer el escalón, que siempre existe.
  }
  enginesCache = { at: Date.now(), byId, modelById };
  return byId;
}

/** Motor y modelo de un agente de Studio, para el pie «agente · modelo · Configurar».
 *  Mismo mapa cacheado (5 min) que `engineOfAgent`: pintar el pie no le pega a Studio. */
export async function agentModelInfo(a: ResolvedAgent): Promise<{ engine: string | null; model: string | null; fleetId: string | null }> {
  if (a.backend.kind !== "fleet") return { engine: null, model: null, fleetId: null };
  await agentEngines();
  return {
    engine: enginesCache?.byId.get(a.backend.id) ?? null,
    model: enginesCache?.modelById.get(a.backend.id) ?? null,
    fleetId: a.backend.id,
  };
}
...
async function runAgentTurnInner(opts: {
  ...
  const segs: string[] = [];
  // Al adoptar, el prefijo es el cuerpo ENTERO de la burbuja muerta: trae sus bloques de
  // estado (plan y checklist) como texto. Se quitan aquí —el plan pasa a ser estado del
  // turno, abajo— o el repintado los duplicaba en crudo y el plan quedaba congelado.
  const planPrevio = opts.prefijo ? extractTodos(opts.prefijo) : null;
  let acc = opts.prefijo ? stripToolBlock(opts.prefijo) : "";
  ...
  let todos: TodoItem[] | null = planPrevio?.todos ?? null;
  let todosAt = planPrevio?.asOf ?? 0;
  ...
  const onTool = async (ev: ToolEvent) => {
    ...
    if (ev.phase !== "end" && ev.name && !toolsCrudas.has(ev.name)) {
      toolsCrudas.add(ev.name);
      opts.onToolNames?.([...toolsCrudas]);
    }
