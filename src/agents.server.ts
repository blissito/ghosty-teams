...
const EB_DOC_STREAM_GUARDRAIL = [
  ...
  "ARTEFACTO INTERACTIVO (app, herramienta, calculadora, visualización, gráfica interactiva, juego, demo, landing con estilo/JS, cualquier cosa que corra en el navegador) → escribe UN SOLO archivo HTML COMPLETO Y AUTOCONTENIDO DENTRO de un bloque que abre con ```eb-artifact y cierra con ```. **OBLIGATORIO — SIEMPRE EN TU RESPUESTA, NUNCA POR TOOL:** el HTML se escribe LITERALMENTE dentro de ese bloque, token por token, como parte del texto que estás respondiendo. PROHIBIDO generar el artefacto con una tool del SDK, escribirlo a un archivo, publicarlo por tu cuenta o anunciarlo con frases tipo «Generando el artefacto…»: eso ESCONDE la construcción y el usuario se queda mirando una pantalla vacía. La plataforma RENDERIZA EN VIVO lo que va apareciendo dentro del bloque — si el HTML no pasa por tu respuesta, NO se ve construir. Escribe el bloque de inmediato y deja que el HTML fluya. **ESTILO CON TAILWIND (obligatorio para el layout/diseño):** incluye `<script src=\"https://cdn.tailwindcss.com\"></script>` en el <head> y estiliza con CLASES de Tailwind en los elementos (bg-*, text-*, flex, grid, gap-*, p-*, rounded-*, etc.) — NO uses estilos inline `style=\"...\"` ni un `<style>` gigante para el layout (el editor visual del panel edita las clases de Tailwind; inline/<style> no es editable ahí). Usa `<style>` SOLO para lo que Tailwind no cubre (keyframes/animaciones custom). El <script> de tu lógica va inline; para React usa Babel-standalone embebido con CDN. Pon <title> para nombrarlo. **TEMA (obligatorio):** DEFINE tú mismo, en un `<style>`, un bloque `:root{}` con una paleta ACORDE a lo pedido usando estos tokens — `--color-background`, `--color-foreground`, `--color-primary`, `--color-primary-foreground`, `--color-secondary`, `--color-secondary-foreground`, `--color-muted`, `--color-muted-foreground`, `--color-accent`, `--color-accent-foreground`, `--color-border`, más `--radius`, `--font-heading` y `--font-body`. Luego USA ESOS TOKENS para TODO el color vía clases arbitrarias de Tailwind (`bg-[var(--color-primary)] text-[var(--color-foreground)] border-[var(--color-border)] text-[var(--color-primary-foreground)]`). Definir la paleta en UN SOLO lugar (`:root`) hace el artefacto AUTOCONTENIDO (se ve igual en su URL pública) Y editable (el panel puede recolorearlo re-mapeando esos tokens). PROHIBIDO regar colores fijos por el markup (`bg-purple-600`, `#7c3aed`, `text-white`, gradientes hardcodeados): todo color sale de un token del `:root`. **RESPONSIVO (obligatorio):** mobile-first y sin desbordes — nada de anchos/altos fijos en px para contenedores, usa `w-full`, `max-w-*`, `mx-auto`, `flex-wrap`, `grid-cols-1` con `sm:`/`md:`/`lg:` para subir de columnas, tipografía escalada (`text-3xl md:text-5xl`), padding responsivo (`px-4 md:px-8`) e imágenes `max-w-full h-auto`. Debe verse bien desde 360px de ancho. Se renderiza EN VIVO en un iframe del panel y se publica con URL compartible. NO uses este bloque para prosa (usa eb-doc) ni para tablas de datos (usa eb-sheet).",
  ...
  "MODIFICAR un ARTEFACTO HTML que YA existe → **EDICIÓN QUIRÚRGICA**. El HTML actual que te paso trae un atributo `data-id` en cada elemento: ESA es la dirección del nodo. Si el cambio afecta a una o pocas zonas concretas (quitar/añadir/editar una tarjeta, cambiar un texto, un precio, el color de un bloque, una fila), NO re-emitas el artefacto entero: manda UN bloque por cada nodo que cambia, así:\n```eb-patch a17\n<div data-id=\"a17\" class=\"…\">…</div>\n```\nHay TRES operaciones y ninguna re-emite el documento: **reemplazar** (```eb-patch <data-id>```), **quitar** (```eb-remove <data-id>``` — una línea, sin HTML) y **agregar** (```eb-insert <data-id-del-ancla> <posición>``` con la posición `append` o `prepend` para colgarlo DENTRO del ancla, o `before`/`after` para ponerlo junto a ella como hermano; el bloque trae el HTML del elemento NUEVO, sin data-id — la plataforma se lo asigna). Para añadir una tarjeta a una rejilla: `eb-insert <id-de-la-rejilla> append`. NUNCA re-emitas el padre entero solo para agregar o quitar un hijo. Reglas del patch: (1) el id de la línea de apertura y el `data-id` del elemento raíz del bloque deben ser EL MISMO; (2) el elemento raíz conserva su MISMA etiqueta; (3) devuelve el nodo COMPLETO (su outerHTML, con todos sus hijos), no un fragmento suelto; (4) CONSERVA los `data-id` de los hijos que no cambian — son direcciones, no adorno; (5) el resultado debe ser 90%+ idéntico al original: el cambio más pequeño que cumpla lo pedido, sin 'mejorar' de paso lo que nadie te pidió; (6) NADA de HTML fuera del bloque, ni explicaciones dentro; (7) para BORRAR un nodo usa el bloque de BORRADO — una sola línea, sin HTML: ```eb-remove a17``` seguido del cierre ``` — NO re-emitas el padre para quitar un hijo (repintaría toda la rejilla); (8) elige el nodo más PEQUEÑO que contenga todo el cambio, y si tocas varias zonas, un bloque por zona; (9) un bloque `eb-insert` puede traer VARIOS elementos hermanos (p.ej. dos tarjetas nuevas) y se insertan todos, pero NUNCA mezcles prosa ni comentarios dentro del bloque: solo HTML.",
  ...
  "EXCEPCIONES (→ NO uses un bloque eb-doc): (a) documentos con membrete de marca fijo; (b) presentaciones (pptx); (c) cuando el usuario pide EXPLÍCITAMENTE un PDF, o un documento 'con diseño'/'vistoso'/'maquetado'/'bonito' → NO lo entregues como eb-doc (eso baja como .docx sin diseño). Para un PDF: arma el HTML con su CSS inline y llama a `publishPdf(html, 'nombre.pdf')` de /opt/gs-sdk/render.mjs — te devuelve el archivo publicado e IMPRIME un bloque que abre con ```eb-file y cierra con ```. DEBES incluir ese bloque TAL CUAL en tu respuesta: así el PDF aparece como tarjeta descargable y se abre en el visor, en vez de quedar como un link suelto. Para una captura, `publishScreenshot(html, 'nombre.png')`. La regla 'toda prosa → eb-doc' aplica al documento por DEFECTO; una petición explícita de PDF/diseño va por publishPdf. Toda tabla/datos → eb-sheet.",
].join(" ");
