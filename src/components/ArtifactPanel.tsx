...
export function officeViewerSrc(src: string): string | null {
  if (!/^https?:\/\//i.test(src) || /\/api\/attachment\//.test(src))
    return null;
        ...
        <thead>
          <tr>
            {head.map((h, i) => (
              <th
                key={i}
                className="border border-neutral-300 bg-neutral-100 px-3 py-1.5 text-left font-semibold"
              >
                {h}
              </th>
            ))}
          ...
          {body.map((r, ri) => (
            <tr key={ri} className={ri % 2 ? "bg-neutral-50" : ""}>
              {head.map((_, ci) => (
                <td
                  key={ci}
                  className="border border-neutral-200 px-3 py-1.5 align-top"
                >
                  {r[ci] ?? ""}
                </td>
              ))}
...
export default function ArtifactPanel({
  ...
  const [sheetState, setSheetState] = useState<"idle" | "loading" | "error">(
    "idle",
  );
  ...
  const [vw, setVw] = useState<number>(() =>
    typeof window === "undefined" ? DEFAULT_W : window.innerWidth,
  );
  ...
  const isXlsx =
    artifact?.kind === "office" && /\.xlsx?$/i.test(artifact.title ?? "");
  const isDocLike =
    artifact?.kind === "doc" ||
  ...
  const editorPatches =
    artifact?.kind === "draft" ? artifact.patches : undefined;
  useEffect(() => {
  ...
  const idxChannelId =
    rootArtifact?.kind === "docindex" ? rootArtifact.channelId : null;
  const idxThreadRootId =
    rootArtifact?.kind === "docindex" ? rootArtifact.threadRootId : undefined;
  ...
  useEffect(() => {
    ...
    listTeamDocumentsFn()
      ...
      .catch(() => {
        if (alive && !docsIndexCache) setIdxDocs([]);
      });
    return () => {
      alive = false;
    };
  ...
  const draftPreview =
    isDraftArtifact && artifact?.kind === "draft" ? artifact.content : "";
  ...
  const patchSig =
    artifact?.kind === "draft"
      ? (artifact.patches ?? []).map((p) => p.nodeId).join(",")
      : "";
  useEffect(() => {
    setFailedPatches([]);
  }, [patchSig]);
  ...
  const doDownload = async (
    formato: "docx" | "xlsx" | "pdf" = artifact?.kind === "sheet"
      ? "xlsx"
      : "docx",
  ) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (alive && s?.slug)
          setShareUrl(
            `${window.location.origin}/artefacto/${s.slug}?v=${v ?? "latest"}`,
          );
      } catch {
    ...
    artifact?.kind === "artifact" ? artifact.versionId : null,
  ]);
  ...
  const openHtmlArtifact = () => {
    ...
    const url = URL.createObjectURL(
      new Blob([artifactHtml ?? artifact.html], { type: "text/html" }),
    );
      ...
      artifact?.kind === "image"
        ...
        : artifact?.kind === "audio"
          ...
          : artifact?.kind === "video"
            ? "mp4"
            : "bin";
    const base = (artifact?.title || "archivo")
  ...
  const doUploadToCase = async (files: FileList | null) => {
    ...
    const slug =
      threadDocs?.[0]?.channelSlug ??
      roomDocs?.[0]?.channelSlug ??
      artifact.channelSlug;
    ...
    try {
      for (const f of Array.from(files)) {
        ...
        await postMessage({
          data: { slug, parentId: null, body: "", attachments: [up] },
        });
      }
  ...
  useEffect(() => {
    const onMove = (e: PointerEvent) => {
      ...
      const w = Math.min(
        Math.max(window.innerWidth - e.clientX, MIN_W),
        maxPanelW(window.innerWidth),
      );
      setWidth(w);
    };
    const onUp = () => {
  ...
  // El contenido pide ancho (hoy: abrir el panel de comentarios de la co-edición). Crece
  // el ARTEFACTO en vez de estrujar el documento, que es lo que se está leyendo. Se clampa
  // igual que el arrastre, así que nunca se come el chat: si no hay espacio, no crece.
  const ajustarAncho = useCallback((delta: number) => {
    setWidth((w) =>
      Math.min(Math.max(w + delta, MIN_W), maxPanelW(window.innerWidth)),
    );
  }, []);
    ...
    <AnimatePresence>
      ...
      {rootArtifact ? (
        <>
          ...
          {overlay ? (
            <div className="hidden shrink-0 lg:block" style={{ width }} />
          ) : null}
          <motion.aside
            className={
              overlay
              ...
              <>
                ...
                {!fullscreen ? (
                  <div
                    ...
                    onDoubleClick={() =>
                      setWidth(Math.min(DEFAULT_W, window.innerWidth - 40))
                    }
                ...
                >
                  ...
                  <ArtifactShareBar
                    ...
                    subtitle={docBadge}
                    // En el panel se trabaja el artefacto (editar, copiar, abrir) y los
                    // permisos se manejan en SU página — por eso normalmente no hay Compartir.
                    // EXCEPCIÓN: en co-edición, invitar a alguien A EDITAR es justo lo que se
                    // quiere hacer desde aquí; mandarte a otra página a media sesión no.
                    documentId={
                      collabMode && artifact.kind === "doc"
                        ? artifact.documentId
                        : null
                    }
                    leading={
                        ...
                        el default mientras se compara contra el editor de siempre. */}
                            {artifact.kind === "doc" ? (
                              <button
                                ...
                                title={
                                  collabMode
                                    ? t("Volver al editor simple")
                                    : t("Editor avanzado y colaborativo")
                                }
                  ...
                  <div className="relative min-h-0 flex-1 overflow-auto bg-surface-3">
                    {artifact.kind === "docindex" ? (
                        ...
                        {dropActive ? (
                          <div className="pointer-events-none absolute inset-0 z-10 grid place-items-center rounded-lg bg-brand/10 text-sm font-semibold text-brand backdrop-blur-[1px]">
                            <span className="inline-flex items-center gap-2">
                              <Upload size={18} />{" "}
                              {t("Suelta para subir a este room")}
                            </span>
                          </div>
                        ) : null}
                      ...
                      <div
                        data-gt-branch="draft-artifact"
                        className="absolute inset-0 flex flex-col bg-surface-2"
                      >
                          ...
                          <div className="flex items-center gap-2 border-b border-border bg-surface px-3 py-1.5 text-xs text-muted">
                            ...
                            {failedPatches.length ? (
                              <span className="rounded bg-red-500/15 px-1.5 py-0.5 text-[11px] text-red-300">
                                {failedPatches.length} {t("sin aplicar")} (
                                {failedPatches.join(", ")})
                              </span>
                            ) : null}
                      ...
                      <div
                        ref={scrollRef}
                        className="min-h-0 flex-1 overflow-auto bg-surface-3 p-4 sm:p-6"
                      >
                            ...
                            sheetState === "loading" ? (
                              ...
                              <div className="grid h-full place-items-center p-6">
                                ...
                                >
                                  ...
                                  <span className="max-w-xs truncate text-sm text-ink">
                                    {artifact.title || t("Documento")}
                                  </span>
                                  <span className="text-xs text-muted">
                                    {t("Descarga el archivo")}
                                  </span>
                                </a>
                              </div>
                            )
                      ...
                      <div className="relative flex h-full flex-col">
                        ...
                        {editing && editorDoc ? (
                          <div className="min-h-0 flex-1">
                            <CanvasEditor
                              ...
                              onSave={async (doc) => {
                                ...
                                setSavedHtml({
                                  id: String(
                                    artifact.messageId ?? artifact.documentId,
                                  ),
                                  html,
                                });
                              }}
                          ...
                          <div
                            data-gt-branch="artifact-final"
                            className="relative min-h-0 flex-1 bg-surface-2"
                          >
                            ...
                            {!(artifactHtml ?? artifact.html ?? "").trim() ? (
                              <ArtifactSkeleton
                                label={t("Construyendo el artefacto…")}
                              />
                            ) : null}
                            <iframe
                      ...
                      <div className="grid min-h-full place-items-center p-6">
                        <div className="w-full max-w-md rounded-xl border border-border bg-surface p-5">
                          <p className="mb-3 text-sm font-medium text-ink">
                            {artifact.question || t("Elige una opción")}
                          </p>
                          <div className="flex flex-col gap-2">
